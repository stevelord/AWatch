...
uint8_t *const nums[] PROGMEM = {spONE, spTWO, spTHREE, spFOUR, spFIVE, spSIX, spSEVEN, spEIGHT, spNINE, spTEN, spELEVEN, spTWELVE, spTHIRTEEN, spFOURTEEN, spFIFTEEN};

char buf[128];
bool rtcIrq = false;

void sayNum(int num, AudioGeneratorTalkie *talkie)
{
  switch(cd){ // I could've done this another way, but it would be further from talkingClockI2S and less readable
    case 1: talkie->say(spONE,sizeof(spONE)); break;
    case 2: talkie->say(spTWO,sizeof(spTWO)); break;
    case 3: talkie->say(spTHREE,sizeof(spTHREE)); break;
    case 4: talkie->say(spFOUR,sizeof(spFOUR)); break;
    case 5: talkie->say(spFIVE,sizeof(spFIVE)); break;
    case 6: talkie->say(spSIX,sizeof(spSIX)); break;
    case 7: talkie->say(spSEVEN,sizeof(spSEVEN)); break;
    case 8: talkie->say(spEIGHT,sizeof(spEIGHT)); break;
    case 9: talkie->say(spNINE,sizeof(spNINE)); break;
    case 10: talkie->say(spTEN,sizeof(spTEN)); break;
    case 11: talkie->say(spELEVEN,sizeof(spELEVEN)); break;
    case 12: talkie->say(spTWELVE,sizeof(spTWELVE)); break;
    case 13: talkie->say(spTHIRTEEN,sizeof(spTHIRTEEN)); break;
    case 14: talkie->say(spFOURTEEN,sizeof(spFOURTEEN)); break;
    case 15: talkie->say(spFIFTEEN,sizeof(spFIFTEEN)); break;
    default:
      break;
  }
  
  //int len = sizeof(nums[num])/sizeof(nums[num][0]);
  //talkie->say((uint8_t *)pgm_read_word(&(nums[num])), sizeof(spTWO));
  //talkie->say(nums[num], len);
  talkie->say(spPAUSE1, sizeof(spPAUSE1));
}
...
void setup()
{
  ...
  ttgo->eTFT->println("The Squawking Clock");  
...
void loop()
{
  ...
  ttgo->eTFT->printf("Count is: %d    ", cd);  
  sayNum(cd, talkie);
  if (cd > 1){
    cd--;
  } else {
    ...
    Serial.printf("\nNow is : %d-%02d-%02d %02d:%02d:%02d\n",
                  curr_datetime.year, curr_datetime.month,
                  curr_datetime.day, curr_datetime.hour,
                  curr_datetime.minute, curr_datetime.second);
    ttgo->eTFT->setCursor(0, 24);                
    ttgo->eTFT->printf("\nThe time is : %d-%02d-%02d %02d:%02d:%02d.   \n",
                  curr_datetime.year, curr_datetime.month,
                  curr_datetime.day, curr_datetime.hour,
                  curr_datetime.minute, curr_datetime.second);                
    sayTime(curr_datetime.hour, curr_datetime.minute, talkie);    
    cd = 15;
  }
