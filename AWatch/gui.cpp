...
LV_IMG_DECLARE(boing);
LV_IMG_DECLARE(alogo);
LV_IMG_DECLARE(disk);
...
static bool alarmLabel_color = false;
//static uint32_t timer_left; // amount of time left in seconds
static uint8_t timer_min; // store of previous time in minutes

static void updateTime();
static void lv_update_task(struct _lv_task_t *);
static void lv_battery_task(struct _lv_task_t *);
static void lv_player_task(struct _lv_task_t *);

//static void modPlayer_task_handle(struct _lv_task_t *);
//lv_task_t *modPlayer_task_handle ;

static void timer_tw_event_cb(uint8_t index); //, bool en);
static void tracker_tw_event_cb(uint8_t index, bool en);
...
//static AudioGeneratorMOD *mod = new AudioGeneratorMOD();;

//unsigned char songs[3] = {*ELYSIUM_MOD, *Chaos_Engine_k8_mod, *Project_X_RE_pxloader_mod};
...
public:
    ...
    void createIcons(lv_obj_t *par)
    {
        ...
        barStyle.body.main_color = LV_COLOR_WHITE;
        barStyle.body.grad_color = LV_COLOR_WHITE;
        barStyle.body.opa = 255;
        barStyle.body.radius = 0;
        barStyle.body.border.color = LV_COLOR_WHITE;
        barStyle.body.border.width = 0;
        barStyle.body.border.opa = LV_OPA_50;
        barStyle.text.color = LV_COLOR_WBLUE;
        barStyle.image.color = LV_COLOR_WBLUE;
        ...
        lv_style_copy(&txtStyle, &barStyle);
        txtStyle.text.font = &Topaz_16;
        ...
        //Fwd and Bwd buttons (decorative versions)
        lv_obj_t * fwdBtn = lv_imgbtn_create(_bar,NULL);
        lv_imgbtn_set_src(fwdBtn, LV_BTN_STATE_REL, &gui_fwd);
        lv_imgbtn_set_src(fwdBtn, LV_BTN_STATE_PR, &gui_fwd);
        lv_imgbtn_set_style(fwdBtn, LV_BTN_STATE_PR, &style_pr);
        lv_obj_align(fwdBtn, _bar, LV_ALIGN_IN_RIGHT_MID, -4, 0); 
        //lv_obj_set_hidden(fwdBtn, true);

        //lv_obj_t * bflBtn = lv_imgbtn_create(_bar,NULL);
        bflBtn = lv_imgbtn_create(_bar,NULL);
        lv_imgbtn_set_src(bflBtn, LV_BTN_STATE_REL, &gui_bwd);
        lv_imgbtn_set_src(bflBtn, LV_BTN_STATE_PR, &gui_bwd);
        lv_imgbtn_set_style(bflBtn, LV_BTN_STATE_PR, &style_pr);
        lv_obj_align(bflBtn, fwdBtn, LV_ALIGN_IN_RIGHT_MID, -24, 0);
        //lv_obj_set_hidden(bflBtn, true);

        _array[0].icon = lv_label_create(_bar, NULL);
        lv_label_set_style(_array[0].icon, LV_LABEL_STYLE_MAIN, &txtStyle);
        lv_label_set_text(_array[0].icon, "100%");
        //lv_label_set_align(_array[0].icon, LV_LABEL_ALIGN_LEFT);
        lv_obj_align(_array[0].icon, bflBtn, LV_ALIGN_IN_RIGHT_MID, -20, 0);
        //lv_obj_set_hidden(_array[0].icon, true);

        _array[1].icon = lv_img_create(_bar, NULL);
        lv_img_set_src(_array[1].icon, LV_SYMBOL_BATTERY_FULL);
        //lv_obj_set_hidden(_array[1].icon, true);      
        ...
        lv_img_set_src(_array[4].icon, LV_SYMBOL_BELL);
        //lv_obj_align(_array[6].icon, _array[1].icon, LV_ALIGN_OUT_RIGHT_MID, -8, 0);
        lv_obj_set_hidden(_array[4].icon, true);

        //Statusbar Menu Button
        //lv_obj_t * menuBtn = lv_imgbtn_create(_bar, NULL);
        bwdBtn = lv_imgbtn_create(_bar, NULL);
        lv_imgbtn_set_src(bwdBtn, LV_BTN_STATE_REL, &gui_close);
        lv_imgbtn_set_src(bwdBtn, LV_BTN_STATE_PR, &gui_close);
        lv_imgbtn_set_src(bwdBtn, LV_BTN_STATE_TGL_REL, &gui_close);
        lv_imgbtn_set_src(bwdBtn, LV_BTN_STATE_TGL_PR, &gui_close);
        lv_imgbtn_set_style(bwdBtn, LV_BTN_STATE_PR, &style_pr);
        //lv_imgbtn_set_style(h_cl_btn, LV_BTN_STATE_TGL_PR, &style_pr);
        //lv_imgbtn_set_toggle(h_cl_btn, true);
        lv_obj_align(bwdBtn, _bar, LV_ALIGN_IN_LEFT_MID, 4, 0);
        lv_obj_set_hidden(bwdBtn, true);

        lv_obj_t * boingLbl = lv_img_create(_bar, NULL);
        lv_img_set_src(boingLbl, &boing);
        //lv_img_set_src(boingLbl, &alogo);
        lv_obj_align(boingLbl, _bar, LV_ALIGN_IN_LEFT_MID, 8, 0);
        //lv_obj_set_hidden(_array[3].icon, true);
        

        //step counter
        _array[5].icon = lv_img_create(_bar, NULL);
        lv_img_set_src(_array[5].icon, LV_SYMBOL_RIGHT);
        lv_obj_align(_array[5].icon, bwdBtn, LV_ALIGN_OUT_RIGHT_MID, 4, 0);
        //lv_obj_set_hidden(_array[4].icon, true);

        _array[6].icon = lv_label_create(_bar, NULL);
        lv_label_set_style(_array[6].icon, LV_LABEL_STYLE_MAIN, &txtStyle);
        lv_label_set_text(_array[6].icon, "0");
        lv_obj_align(_array[6].icon, _array[5].icon, LV_ALIGN_OUT_RIGHT_MID, 4, 0);
        //lv_obj_set_hidden(_array[5].icon, true);
    ...
    {
        lv_label_set_text(_array[6].icon, String(counter).c_str());
        ...
        for (int i = 0; i < 5; i++) {
            if (!lv_obj_get_hidden(_array[i].icon)) {
                if (i == LV_STATUS_BAR_BATTERY_LEVEL) {
                    //lv_obj_align(_array[i].icon, bwdBtn, LV_ALIGN_IN_RIGHT_MID, -5, 0);
                    //lv_obj_align(_array[i].icon, _array[0].icon, LV_ALIGN_IN_RIGHT_MID, iconOffset, 0);
                } else {
...
public:
    ...
    void createMenu(lv_menu_config_t *config, int count, lv_event_cb_t event_cb, int direction = 1)
    {
        ...
        menuStyle.body.main_color = LV_COLOR_WBLUE;
        menuStyle.body.grad_color = LV_COLOR_WBLUE;
        menuStyle.body.opa = LV_OPA_COVER;
        menuStyle.body.radius = 0;
        menuStyle.body.border.color = LV_COLOR_WHITE;
        menuStyle.body.border.width = 0;
        menuStyle.body.border.opa = LV_OPA_COVER;
        menuStyle.text.color = LV_COLOR_WHITE;
        menuStyle.text.font = &Topaz_16;
        //menuStyle.image.color = LV_COLOR_WBORA;
        menuStyle.image.opa = LV_OPA_COVER;

        static lv_style_t iconStyle;
        lv_style_copy(&iconStyle, &menuStyle);
        iconStyle.body.border.width = 0;
        iconStyle.body.border.color = LV_COLOR_WBORA;
        ...
        lv_obj_set_size(_cont, LV_HOR_RES, LV_VER_RES - 20);
        lv_obj_align(_cont, NULL, LV_ALIGN_CENTER, 0, 0);
        ...
        for (int i = 0; i < count; i++) {
            ...
            lv_img_set_style(img, LV_IMG_STYLE_MAIN, &iconStyle);
            ...
            lv_obj_align(label, img, LV_ALIGN_OUT_BOTTOM_MID, 0, 8);
        ...
        lv_imgbtn_set_src(_exit, LV_BTN_STATE_REL, &gui_close);
        lv_imgbtn_set_src(_exit, LV_BTN_STATE_PR, &gui_close);
        lv_imgbtn_set_src(_exit, LV_BTN_STATE_TGL_REL, &gui_close);
        lv_imgbtn_set_src(_exit, LV_BTN_STATE_TGL_PR, &gui_close);
        lv_obj_align(_exit, bwdBtn, LV_ALIGN_IN_RIGHT_MID, 0, 0);
...
void setupGui()
{
    ...
    settingStyle.body.main_color = LV_COLOR_WBLUE;
    settingStyle.body.grad_color = LV_COLOR_WBLUE;
    ...
    lv_style_copy(&bgStyle, &lv_style_plain_color);
    bgStyle.body.main_color = LV_COLOR_WBLUE;
    bgStyle.body.grad_color = LV_COLOR_WBLUE;
    bgStyle.body.border.color = LV_COLOR_WHITE;
    bgStyle.body.border.width = 2;
    bgStyle.body.border.opa = LV_OPA_COVER;
    bgStyle.body.opa = LV_OPA_COVER;
    ...
    mainStyle.body.main_color = LV_COLOR_WBLUE;
    mainStyle.body.grad_color = LV_COLOR_WBLUE;
    mainStyle.body.opa = LV_OPA_COVER;
    mainStyle.body.radius = 0;
    mainStyle.body.border.color = LV_COLOR_WHITE;
    mainStyle.body.border.width = 2;
    mainStyle.body.border.opa = LV_OPA_COVER;
    mainStyle.text.color = LV_COLOR_WHITE;
    mainStyle.text.font = &Topaz_16;
    ...
    timeStyle.text.font = &digital_font;

    //! Date
    
    lv_style_copy(&dateStyle, &mainStyle);
    dateStyle.text.font = &Topaz_16;

    lv_style_copy(&inv_dateStyle1, &dateStyle);
    inv_dateStyle1.text.color = LV_COLOR_WBORA;

    lv_style_copy(&inv_dateStyle2, &dateStyle);
    inv_dateStyle2.text.color = LV_COLOR_BLACK;
    ...
    alarmLabel = lv_label_create(mainBar, NULL);
    ...
    lv_imgbtn_set_src(menuBtn, LV_BTN_STATE_REL, &disk);
    lv_imgbtn_set_src(menuBtn, LV_BTN_STATE_PR, &disk);
    lv_imgbtn_set_src(menuBtn, LV_BTN_STATE_TGL_REL, &disk);
    lv_imgbtn_set_src(menuBtn, LV_BTN_STATE_TGL_PR, &disk);
    lv_imgbtn_set_style(menuBtn, LV_BTN_STATE_PR, &style_pr);
    lv_obj_align(menuBtn, alarmLabel, LV_ALIGN_OUT_BOTTOM_MID, 0, 16);
    lv_obj_set_event_cb(menuBtn, event_handler);
    //lv_obj_set_event_cb(h_cl_btn, event_handler);

    diskLabel = lv_label_create(mainBar, NULL);
    lv_label_set_style(diskLabel, LV_LABEL_STYLE_MAIN, &dateStyle);
    lv_label_set_text(diskLabel, "Workbench");
    lv_obj_align(diskLabel, menuBtn, LV_ALIGN_OUT_BOTTOM_MID, 0, 8);

    lv_task_create(lv_update_task, 1000, LV_TASK_PRIO_LOWEST, NULL);
    lv_task_create(lv_battery_task, 30000, LV_TASK_PRIO_LOWEST, NULL);
    //lv_task_create(lv_player_task, 5, LV_TASK_PRIO_LOWEST, NULL);
}
...
static void updateTime()
{
    ...
    lv_obj_align(dateLabel, timeLabel, LV_ALIGN_OUT_BOTTOM_MID, 0, 16);
    
    // Timer stuff
    if (timer_active){
      if (timer_left > 0){
        //Serial.printf("timer_left: %u | time: %u:%u:%u\n", info.tm_min, timer_min, timer_left, info.tm_hour, info.tm_min, info.tm_sec);
        sprintf(tbuf,"%u mins, %u secs left\n", timer_left/60, timer_left % 60);
        //timer_left--; //delete once working on awatch
      } else {
    ...
    if (timer_active || alarm_active){
        if (alarmLabel_color){
        lv_label_set_style(alarmLabel, LV_LABEL_STYLE_MAIN, &inv_dateStyle1);
      } else {
        lv_label_set_style(alarmLabel, LV_LABEL_STYLE_MAIN, &inv_dateStyle2);
      }
      alarmLabel_color = !alarmLabel_color;
    }
    if (!timer_active && !alarm_active) {
      sprintf(tbuf,"Timer off");
      lv_label_set_style(alarmLabel, LV_LABEL_STYLE_MAIN, &dateStyle);
...
static void lv_player_task(struct _lv_task_t *data)
{
...
public:
    ...
    void create(TimerWindow_cfg_t *cfg, uint8_t count, exit_cb cb, lv_obj_t *parent = nullptr)
    {
        static lv_style_t twlStyle;
        lv_style_copy(&twlStyle, &lv_style_plain);    /*Copy a built-in style to initialize the new style*/
        twlStyle.body.main_color = LV_COLOR_WBLUE;
        twlStyle.body.grad_color = LV_COLOR_WBLUE;
        twlStyle.body.padding.inner = 4;
        twlStyle.body.padding.top = 0;
        twlStyle.body.padding.bottom = 0;
        twlStyle.body.opa = LV_OPA_COVER;
        twlStyle.body.radius = 0;
        twlStyle.body.border.color = LV_COLOR_WHITE;
        twlStyle.body.border.width = 2;
        twlStyle.body.border.opa = LV_OPA_COVER;
        twlStyle.text.color = LV_COLOR_WHITE;
        twlStyle.text.font = &Topaz_16;
        twlStyle.image.color = LV_COLOR_WHITE;

        static lv_style_t inv_twlStyle;
        lv_style_copy(&inv_twlStyle, &twlStyle);    /*Copy a built-in style to initialize the new style*/
        inv_twlStyle.body.main_color = LV_COLOR_WBORA;
        inv_twlStyle.body.grad_color = LV_COLOR_WBORA;
        inv_twlStyle.text.color = LV_COLOR_BLACK;
        inv_twlStyle.image.color = LV_COLOR_WBORA;        
        ...
        _twCont = lv_cont_create(parent, NULL);
        lv_obj_set_size(_twCont, LV_HOR_RES, LV_VER_RES - 24);
        lv_obj_align(_twCont, NULL, LV_ALIGN_CENTER, 0, 0);
        lv_obj_set_style(_twCont, &twlStyle);

        _count = count;
        _tw = new lv_obj_t *[count];
        _cfg = new TimerWindow_cfg_t [count];

        memcpy(_cfg, cfg, sizeof(TimerWindow_cfg_t) * count);

        lv_obj_t *prev = nullptr;
        for (int i = 0; i < count; i++) {
            _tw[i] = lv_btn_create(_twCont, NULL);
            lv_obj_t *la1 = lv_label_create(_tw[i], NULL);
            ...
            lv_imgbtn_set_style(_tw[i], LV_BTN_STATE_REL, &twlStyle);
            lv_imgbtn_set_style(_tw[i], LV_BTN_STATE_PR, &inv_twlStyle);
            lv_imgbtn_set_style(_tw[i], LV_BTN_STATE_TGL_REL, &twlStyle);
            lv_imgbtn_set_style(_tw[i], LV_BTN_STATE_TGL_PR, &inv_twlStyle);
            /*lv_imgbtn_set_src(_tw[i], LV_BTN_STATE_REL, &off);
            lv_imgbtn_set_src(_tw[i], LV_BTN_STATE_PR, &off);
            lv_imgbtn_set_src(_tw[i], LV_BTN_STATE_TGL_REL, &off);
            lv_imgbtn_set_src(_tw[i], LV_BTN_STATE_TGL_PR, &off);*/
            //lv_btn_set_toggle(_tw[i], true);
            //lv_obj_align(_tw[i], la1, LV_ALIGN_IN_TOP_MID, 0, 0);
            lv_obj_set_height(_tw[i],45);
            //i == 0 ? lv_obj_align(_tw[i], NULL, LV_ALIGN_IN_TOP_MID, 0, 20) : lv_obj_align(_tw[i], prev, LV_ALIGN_OUT_BOTTOM_MID, 0, 40);
            i == 0 ? lv_obj_align(_tw[i], NULL, LV_ALIGN_IN_TOP_MID, 0, 5) : lv_obj_align(_tw[i], prev, LV_ALIGN_OUT_BOTTOM_MID, 0, 25);
            lv_obj_set_event_cb(_tw[i], __TimerWindow_event_cb);
            prev = la1;
        }

        _exitBtn = lv_imgbtn_create(_twCont, NULL);
        ...
        lv_obj_align(_exitBtn, _twCont, LV_ALIGN_IN_BOTTOM_MID, 0, -10);
        lv_obj_set_event_cb(_exitBtn, __TimerWindow_event_cb);
    ...
    void align(const lv_obj_t *base, lv_align_t align, lv_coord_t x = 0, lv_coord_t y = 0)
    {
        lv_obj_align(_twCont, base, align, x, y);
    }

    void hidden(bool en = true)
    {
        lv_obj_set_hidden(_twCont, en);
    }

    static void __TimerWindow_event_cb(lv_obj_t *obj, lv_event_t event)
    {
        if (event == LV_EVENT_SHORT_CLICKED) {
            if (obj == _TimerWindow->_exitBtn) {
                if ( _TimerWindow->_exit_cb != nullptr) {
                    _TimerWindow->_exit_cb();
                    return;
                }
            } else {
              for (int i = 0; i < _TimerWindow->_count ; i++) {
                  lv_obj_t *tw = _TimerWindow->_tw[i];
                  if (obj == tw) {
                      if (_TimerWindow->_cfg[i].cb != nullptr) {
                          _TimerWindow->_cfg[i].cb(i);
                      }
                      return;
                  }
              }
        ...
        /*if (event == LV_EVENT_VALUE_CHANGED) {
            for (int i = 0; i < _TimerWindow->_count ; i++) {
                lv_obj_t *tw = _TimerWindow->_tw[i];
                if (obj == tw) {
                    const void *src =  lv_imgbtn_get_src(tw, LV_BTN_STATE_REL);
                    const void *dst = src == &off ? &on : &off;
                    bool en = src == &off;
                    lv_imgbtn_set_src(tw, LV_BTN_STATE_REL, dst);
                    lv_imgbtn_set_src(tw, LV_BTN_STATE_PR, dst);
                    lv_imgbtn_set_src(tw, LV_BTN_STATE_TGL_REL, dst);
                    lv_imgbtn_set_src(tw, LV_BTN_STATE_TGL_PR, dst);
                    if (_TimerWindow->_cfg[i].cb != nullptr) {
                        _TimerWindow->_cfg[i].cb(i, en);
                    }
                    return;
                }
            }
...
private:
    static TimerWindow *_TimerWindow;
    lv_obj_t *_twCont = nullptr;
    uint8_t _count;
    lv_obj_t **_tw = nullptr;
    TimerWindow_cfg_t *_cfg = nullptr;
    lv_obj_t *_exitBtn = nullptr;
    exit_cb _exit_cb = nullptr;
};

TimerWindow *TimerWindow::_TimerWindow = nullptr;
...
public:
    ...
    void create(switch_cfg_t *cfg, uint8_t count, exit_cb cb, lv_obj_t *parent = nullptr)
    {
        ...
        swlStyle.body.main_color = LV_COLOR_WBLUE;
        swlStyle.body.grad_color = LV_COLOR_WBLUE;
        swlStyle.body.opa = LV_OPA_COVER;
        swlStyle.body.radius = 0;
        swlStyle.body.border.color = LV_COLOR_WHITE;
        swlStyle.body.border.width = 2;
        swlStyle.body.border.opa = LV_OPA_COVER;
        ...
        lv_obj_set_size(_swCont, LV_HOR_RES, LV_VER_RES - 24);
        ...
        for (int i = 0; i < count; i++) {
            ...
            lv_obj_align(_sw[i], la1, LV_ALIGN_OUT_RIGHT_MID, 60, 0);
...
public:
    ...
    void create(lv_obj_t *parent = nullptr)
    {
        ...
        if (_preloadCont == nullptr) {
            ...
            plStyle.body.main_color = LV_COLOR_WBLUE;
            plStyle.body.grad_color = LV_COLOR_WBLUE;
            plStyle.body.opa = LV_OPA_0;
            plStyle.body.radius = 0;
            plStyle.body.border.color = LV_COLOR_WHITE;
            plStyle.body.border.width = 2;
...
static void tracker_tw_event_cb(uint8_t index)
{
    
    //file = new AudioFileSourcePROGMEM( &songs[index]) , sizeof(&songs[index]) );
    //file = new AudioFileSourcePROGMEM( ELYSIUM_MOD) , sizeof(ELYSIUM_MOD) );
    if (mod_mode > 0 && song_index == index){
      mod_mode = 0;
    } else if (mod_mode > 0 && song_index != index){
      song_index = index;
      mod_mode = 1;      
    } else {
      song_index = index;
    ...
    Serial.printf("Tracker alert: option %u, mod_mode %u\n", index, mod_mode);
...
static void tracker_event_cb()
{
    ...
    TimerWindow::TimerWindow_cfg_t cfg[cfg_count] = {
        {"Elysium", tracker_tw_event_cb},
        {"Chaos Engine", tracker_tw_event_cb},
        {"Project-X", tracker_tw_event_cb},
    };
    tw = new TimerWindow;
 ...
 *
 */
static void timer_tw_event_cb(uint8_t index)
{
    //Add timers that need to be controlled
    Serial.printf("Timer alert: %u - %u\n", index, timer_left);
    ...
    switch(index){
      ...
      case 1:
        ...
        timer_active = false;
        alarm_active = false;
        Serial.println("Cancelled timer.");
        break;
      default:
        break;
    }
...
static void ex_table_create(nCovData_t *data)
{
    ...
    style_cell1.text.font = &Topaz_16;
    ...
    style_cell2.text.font = &Topaz_16;
