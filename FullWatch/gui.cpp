...
public:
    ...
    void createIcons(lv_obj_t *par)
    {
        ...
        barStyle.body.main_color = LV_COLOR_BLACK;
        barStyle.body.grad_color = LV_COLOR_BLACK;
        ...
        lv_img_set_src(_array[1].icon, LV_SYMBOL_BATTERY_FULL); // LV_SYMBOL_BATTERY_FULL

        _array[2].icon = lv_img_create(_bar, NULL);
        lv_img_set_src(_array[2].icon, LV_SYMBOL_WIFI); // LV_SYMBOL_WIFI
        lv_obj_set_hidden(_array[2].icon, true);

        _array[3].icon = lv_img_create(_bar, NULL);
        lv_img_set_src(_array[3].icon, LV_SYMBOL_BLUETOOTH); // LV_SYMBOL_BLUETOOTH
        ...
        //lv_label_set_text(_array[5].icon, "AWatch.");
        //lv_obj_align(_array[5].icon, _bar, LV_ALIGN_IN_TOP_LEFT, 2, 2);

        //Close Button - not needed for root window
        lv_obj_t * h_cl_btn = lv_imgbtn_create(_bar, NULL);
        ...
        lv_obj_align(h_cl_btn, _bar, LV_ALIGN_IN_TOP_LEFT, 4, 0);
...
public:
    ...
    void createMenu(lv_menu_config_t *config, int count, lv_event_cb_t event_cb, int direction = 1)
    {
        ...
        lv_obj_align(_cont, NULL, LV_ALIGN_OUT_BOTTOM_MID, 0, -32);
...
MenuBar::lv_menu_config_t _cfg[2] = {
    ...
    if (event == LV_EVENT_SHORT_CLICKED) {  //!  Event callback Is in here
        if (obj == menuBtn) {
            ...
            if (menuBars.self() == nullptr) {
                ...
                lv_obj_align(menuBars.self(), bar.self(), LV_ALIGN_OUT_BOTTOM_MID, -8, -8);
...
void setupGui()
{
    Serial.println("setupGui() entered.");

    TTGOClass *ttgo = TTGOClass::getWatch();
    Serial.println("getWatch() called.");
   

    lv_style_copy(&settingStyle, &lv_style_plain);    /*Copy a built-in style to initialize the new style*/
    settingStyle.body.main_color = LV_COLOR_RED;
    settingStyle.body.grad_color = LV_COLOR_RED;
    ...
    lv_obj_set_style(scr, &settingStyle);
    ...
    Serial.println("Bar added. Calling getWatch().");

    if (ttgo->power->isChargeing()) {
        icon = LV_ICON_CHARGE;
    }
    updateBatteryIcon(icon);
    Serial.println("Battery complete. Oldstyle code next.");

    //! main
    static lv_style_t mainStyle;
    lv_style_copy(&mainStyle, &lv_style_plain);    /*Copy a built-in style to initialize the new style*/
    mainStyle.body.main_color = LV_COLOR_WHITE;
    mainStyle.body.grad_color = LV_COLOR_WHITE;
    ...
    updateTime();
    Serial.println("updateTime() returned. More style shiz.");
    //! menu
    static lv_style_t style_pr;
    lv_style_copy(&style_pr, &lv_style_plain);
    style_pr.image.color = LV_COLOR_BLACK;
...
static void updateTime()
{
    Serial.println("Calling RTC->getDateTime()");
    TTGOClass *ttgo = TTGOClass::getWatch();
    RTC_Date curr_datetime = ttgo->rtc->getDateTime();
    Serial.println("Time got. Setting labels.");
    ...
    Serial.println("Labels set, returning after dead oldcode.");
    //---------------------- Old code    
    /*
    time_t now;
    struct tm  info;
